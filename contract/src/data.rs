...
impl BrigdePool {
    pub fn instance() -> BrigdePool {
        ...
        let key = runtime::get_key(HASH_ADDR_LIQUIDITIES_DICT).unwrap_or_revert_with(Error::HASH_ADDR_LIQUIDITIES_DICTFail);
        let key = runtime::get_key(ALLOWED_TARGETS_DICT).unwrap_or_revert_with(Error::ALLOWED_TARGETS_DICTFail);
        let key = runtime::get_key(USED_HASHES_DICT).unwrap_or_revert_with(Error::USED_HASHES_DICTFail);
        let key = runtime::get_key(SIGNERS_DICT).unwrap_or_revert_with(Error::SIGNERS_DICTFail);
        let key = runtime::get_key(TOKEN_CONTRACT_PACKAGE_HASH_DICT_NAME).unwrap_or_revert_with(Error::TOKEN_CONTRACT_PACKAGE_HASH_DICT_NAMEFail);
        BrigdePool {
