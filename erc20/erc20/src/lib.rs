...
impl ERC20 {
    ...
    fn read_allowance(&self, owner: Address, spender: Address) -> U256 {
        let allowance_uref = detail::get_uref(ALLOWANCES_KEY_NAME);
        allowances::read_allowance_from(allowance_uref, owner, spender)
    }

    fn write_allowance(&mut self, owner: Address, spender: Address, amount: U256) {
        let allowance_uref = detail::get_uref(ALLOWANCES_KEY_NAME);
        allowances::write_allowance_to(allowance_uref, owner, spender, amount)
    }
    ...
    pub fn transfer_from(
        ...
        let spender = detail::get_immediate_caller_address().unwrap_or_revert_with(Error::User(8000u16));
        if amount.is_zero() {
            return Ok(());
        }
