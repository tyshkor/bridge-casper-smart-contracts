...
mod tests {
    ...
    #[test]
    fn should_be_able_to_install_and_add_liquidity_and_signed_experiment() {
        let mut builder = InMemoryWasmTestBuilder::default();
        builder
            .run_genesis(&PRODUCTION_RUN_GENESIS_REQUEST)
            .commit();
        ...
        let signed_experiment_args = runtime_args! {
            "token_address" => erc20_contract_package_hash_string,
            "payee" => payee,
            "amount" => amount,
            "chain_id" => chain_id,
    ...
    #[test]
    fn should_be_able_to_install_and_add_liquidity_and_signed_basic() {
        let mut builder = InMemoryWasmTestBuilder::default();
        builder
            .run_genesis(&PRODUCTION_RUN_GENESIS_REQUEST)
            .commit();
        ...
        let signed_basic_args = runtime_args! {
            "signature" => signature_string,
            "hash" => message_hash,
        };

        let signed_basic_request = ExecuteRequestBuilder::contract_call_by_hash(
            *DEFAULT_ACCOUNT_ADDR,
            bridge_pool_contract_hash,
            "signed_basic",
            signed_basic_args,
